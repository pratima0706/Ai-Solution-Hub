...
def admin_dashboard(request):
    ...
    unsubscribed_7d = NewsletterSubscriber.objects.filter(
        is_active=False,
        unsubscribed_at__gte=timezone.now() - timedelta(days=7)
    ).count()
    ...
    recent_subscribers = NewsletterSubscriber.objects.filter(is_active=True).order_by('-subscribed_at')[:10]
    ...
    context = {
        ...
        'unsubscribed_7d': unsubscribed_7d,
        'weekly_contacts': list(weekly_contacts),
        'weekly_subscribers': list(weekly_subscribers),
